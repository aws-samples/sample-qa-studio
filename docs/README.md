# QA Studio Documentation

Complete documentation for QA Studio - an AI-powered automated testing platform built on Amazon Nova Act, supporting both web browser and mobile app testing via AWS Device Farm.

## Quick Links

- [User Guide](user-guide.md) - Complete walkthrough of the web interface
- [Architecture](architecture.md) - System design and data flows
- [API Reference](api-reference.md) - Complete API documentation
- [Best Practices](best-practices.md) - Testing and security guidance
- [Troubleshooting](troubleshooting.md) - Common issues and solutions

## Getting Started

### Web Application
Deploy the full QA Studio platform to AWS. See [Web App README](../web-app/README.md) for deployment instructions.

### CLI Tool
Install the command-line tool for local development and test execution. See [CLI README](../qa-studio-cli/README.md) for installation and usage.

### Agent Skill
Install the QA Studio agent skill for Kiro IDE and other coding agents using `npx skills add`. See [CLI README](../qa-studio-cli/README.md#kiro-ide-skill-installation) for setup instructions.

## Documentation

### User Guides
- [User Guide](user-guide.md) - Complete walkthrough of the web interface
- [Prompting Best Practices](prompting-best-practices.md) - Writing effective test steps
- [Best Practices](best-practices.md) - Test organization, security, and performance

### Technical Reference
- [Architecture](architecture.md) - System design and data flows
- [API Reference](api-reference.md) - Complete API documentation
- [Development Guide](development.md) - Development workflows and commands

### Troubleshooting
- [Troubleshooting Guide](troubleshooting.md) - Common issues and solutions

## Support

- **GitHub Issues**: [Report issues](https://github.com/aws-samples/sample-nova-act-qa-studio/issues)
- **Documentation**: [Full documentation](https://github.com/aws-samples/sample-nova-act-qa-studio)

## License

This project is licensed under the MIT-0 License.
