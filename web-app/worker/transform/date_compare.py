...
The :mod:`transform.date_parser` module handles parsing. This module
layers operator dispatch, the ``equals_within`` JSON payload, and the
naive-vs-aware warning on top, so the worker's assertion_step and
validation_step (and their CLI mirrors) share a single comparison
surface.
...
# Operator → human-readable failure description (template).
