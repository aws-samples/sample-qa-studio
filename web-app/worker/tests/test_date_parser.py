"""Tests for the centralized date parser (Task 1)."""
...
class TestAutoDetectIso:
    ...
    def test_datetime_with_zero_offset(self):
        dt, naive = parse_to_utc("2024-01-02T15:04:05+00:00")
        assert dt == datetime(2024, 1, 2, 15, 4, 5, tzinfo=UTC)
        assert naive is False
    ...
    def test_datetime_with_space_separator(self):
        # ISO 8601 permits a space in place of T (per RFC 3339 §5.6).
...
class TestAutoDetectEpoch:
    ...
    def test_epoch_seconds_min_boundary(self):
        # 10 digits, lowest: 1000000000 = 2001-09-09 01:46:40 UTC
    ...
    def test_epoch_takes_precedence_over_iso_basic_format(self):
        ...
        assert dt.year < 9000  # not parsed as the ISO-basic year 8796
        assert naive is False
...
class TestAmbiguousFails:
    @pytest.mark.parametrize("value", [
        ...
        "Jan 2 2024",
        "2 Jan 2024",
        "02-01-2024",          # not ISO (DD-MM-YYYY)
        "2024",                # year alone
        "abc",
        "12345",               # 5 digits — neither epoch shape
        "123456789",           # 9 digits — neither epoch shape
        "12345678901",         # 11 digits — neither epoch shape
        "12345678901234",      # 14 digits — neither epoch shape
    ])
...
class TestExplicitFormat:
    ...
    def test_short_month(self):
        dt, naive = parse_to_utc("Jan 2, 2024", format="%b %d, %Y")
        assert dt == datetime(2024, 1, 2, tzinfo=UTC)
        assert naive is True
    ...
    def test_format_mismatch_includes_value(self):
        with pytest.raises(DateParseError) as exc_info:
...
class TestTimezoneNormalization:
    ...
    def test_strptime_naive_anchored_to_utc(self):
        dt, naive = parse_to_utc("02/01/2024 15:00", format="%d/%m/%Y %H:%M")
        assert dt.tzinfo == UTC
        assert naive is True

    def test_strptime_aware_converted_to_utc(self):
        dt, naive = parse_to_utc("02/01/2024 15:00 -0500", format="%d/%m/%Y %H:%M %z")
        assert dt.hour == 20  # 15:00 EST = 20:00 UTC
        assert dt.tzinfo == UTC
        assert naive is False
