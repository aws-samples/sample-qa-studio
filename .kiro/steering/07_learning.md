...
* Nova Act artifacts are stored at `{usecase_id}/{execution_id}/recording/act_{act_id}.json` in S3, not `executions/{execution_id}/act_{act_id}.json`. The prefix is set in worker.py when creating the browser session.
