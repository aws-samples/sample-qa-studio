...
Adapted from worker/nova_act_workflow.py for the CI/CD runner context.
"""
    ...
    def ensure_workflow(self, usecase_id: str) -> str:
        ...
        try:
            try:
                ...
                logger.info(
                    f"Workflow '{workflow_name}' exists (ID: {resp.get('id', 'unknown')})"
                )
                ...
                logger.warning(
                    f"Could not check workflow definition: {e}. Will attempt to use it anyway."
                )
        except Exception as e:
