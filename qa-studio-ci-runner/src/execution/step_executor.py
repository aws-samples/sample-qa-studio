...
Adapted from the worker's individual step files into a single cohesive class.
"""
...
class StepExecutor:
    ...
    def execute(
        ...
        match step_type:
            ...
            case _:
                logger.warning(f"Unknown step_type '{step_type}', falling back to navigation")
                return self._execute_navigation(step)
    ...
    def _execute_assertion(self, step: dict, runtime_variables: dict) -> StepResult:
        ...
        if var_name not in runtime_variables:
            return StepResult(
                success=False,
                logs=f"Runtime variable '{var_name}' not found",
    ...
    def _execute_secret(self, step: dict) -> StepResult:
        ...
        try:
            ...
            if secret_value is None:
                return StepResult(
                    success=False,
                    logs=f"Secret key '{secret_key}' not found",
                )
            ...
            # Type the secret value into the focused field
            self.nova.page.keyboard.type(secret_value)

            if not result.parsed_response:
                return StepResult(
                    success=False, act_id=act_id,
                    logs=f"Secret step action was not successful",
                )
    ...
    def _execute_download(self, step: dict) -> StepResult:
        ...
        try:
            # Set up CDP interception for downloads
    ...
    def _download_from_stream(self, cdp_session, interception_id: str, filename: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error downloading from stream: {e}")
            return None
    ...
    def _compare(validation_type: str, operator: str, expected_raw, actual_raw) -> bool:
        ...
        match operator:
            ...
            case "contains":
                return bool(re.search(re.escape(expected_str), actual_str))
            case "contains_case_insensitive":
                return bool(re.search(re.escape(expected_str), actual_str, re.IGNORECASE))
            case "not_equal":
                return actual_str != expected_str
            case _:
                return actual_str == expected_str
