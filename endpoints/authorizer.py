    ...
    try:
        ...
        try:
            ...
            if email:
                ...
                identity_type = 'user'
            elif 'api/execute' in scope:
                # M2M token with execute scope
                principal_id = client_id or sub
                identity_type = 'client'
            else:
                # Unknown token type
                logger.warning(f"Unknown token type - no email/username and no api/execute scope")
                raise Exception('Unauthorized')
            
            logger.info(f"Authorized: {principal_id} (type: {identity_type})")
